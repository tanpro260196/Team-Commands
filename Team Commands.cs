            ...
            if (e.Parameters.Count == 0) e.Player.SendErrorMessage("Invalid syntax! Proper syntax: /teamcmd <command>. Type /team help for all commands.");
                        ...
                        if (e.Parameters[1].Length == 0) e.Player.SendErrorMessage("Missing player name.");
                            ...
                            foreach (TSPlayer player in TShock.Players)
                                if (player != null && (team == player.Team || team == 5))
                                    player.Kick( reason, !e.Player.RealPlayer, true, e.Player.Name);
                    ...
                    if (CommandIsValid(e, Permissions.ban, 2, "ban <team> [reason]")) {
                        ...
                        foreach (TSPlayer player in TShock.Players)
                            if (player != null && (team == player.Team || team == 5) && (!player.Group.HasPermission(Permissions.immunetoban) || e.Player.RealPlayer)) {
                                var user = TShock.UserAccounts.GetUserAccountByID(player.Index);
                                var knownIps = JsonConvert.DeserializeObject<List<string>>(user.KnownIps);
                                TShock.Bans.AddBan(knownIps.Last(), user.Name, user.UUID, user.Name, reason, false, e.Player.Account.Name);
                            }
                ...
                case "tempban":
                    if (CommandIsValid(e, Permissions.ban, 3, "tempban <team> <time> [reason]")) {
                        ...
                        foreach (TSPlayer player in TShock.Players)
                            if (player != null && (team == player.Team || team == 5) && (!player.Group.HasPermission(Permissions.immunetoban) || e.Player.RealPlayer))
                                    if (TShock.Bans.AddBan(player.IP, player.Name, player.UUID, player.Name, reason, false, e.Player.Name, DateTime.UtcNow.AddSeconds(time).ToString("s")))
                ...
                case "tempgroup":
                    if (CommandIsValid(e, Permissions.settempgroup, 3, "tempgroup <team> <new group>")) {
                        ...
                        if (!TShock.Groups.GroupExists(e.Parameters[2]))
                            ...
                            var g = TShock.Groups.GetGroupByName(e.Parameters[2]);
                            foreach (TSPlayer player in TShock.Players)
                                if (player != null && (team == player.Team || team == 5)) {
                                    player.tempGroup = g;
